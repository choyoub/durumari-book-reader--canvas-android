...
async function textFromZip(bytes: Uint8Array) {
  ...
  for (const entry of entries) {
    ...
    if (/\.epub$/i.test(entry.name)) {
      parts.push(`\n\n${entry.name}\n\n${await textFromEpub(content)}`);
      continue;
    }
    parts.push(entries.length > 1 ? `\n\n${entry.name}\n\n${decodeText(content)}` : decodeText(content));
  }
  return parts.join("\n");
}

async function extractText(kind: BookKind, bytes: Uint8Array) {
  if (kind === "txt") return decodeText(bytes);
  if (kind === "gz") return decodeText(ungzip(bytes));
  if (kind === "zip") return textFromZip(bytes);
